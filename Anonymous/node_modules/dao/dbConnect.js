...
function Display(client,callback){
    ...
    var delete_stmt = "Delete from anonymous WHERE expire_time < ?";

    client.query(delete_stmt, myDate, function(err, result){
        if(err){
...
function DisplayUser(client,un,callback){
    client.query('select file_name, path,tag1,tag2,tag3 from file where uploader =? ',[un],
        function(err,result){
            if(err)
                throw err;
            else
...
function DisplayFilePool(client,un,callback){
	var myDate =  moment(new Date()).format("YYYY-MM-DD HH:mm:ss");
...
function Search(client,search_content,username,loginState,callback){
    ...
    var select_stmt_part2 = "SELECT * FROM anonymous ";
    var results = [];
    ...
    var formated_now = moment(new Date()).format(right_now);
    ...
    if(search_content.length == 64){
        ...
        var select_stmt8 = "(" + select_stmt_part2 + "WHERE find_in_set('"+search_content+"', tag1) OR find_in_set('"+search_content+"',tag2) OR find_in_set('"+search_content+"',tag3) LIMIT 6)";
        ...
        var final = [];
        var finalStatement = select_stmt1 + unionString + select_stmt2 + unionString + select_stmt3 + unionString + select_stmt4 + unionString + select_stmt5 + unionString + select_stmt6 + unionString + select_stmt7 + unionString + select_stmt8;
